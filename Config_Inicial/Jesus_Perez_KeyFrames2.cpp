...
void resetElements(void)
{
	...
	BRLegs = KeyFrame[0].BRLegs;
	BLLegs = KeyFrame[0].BLLegs;
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].BRLegsInc = (KeyFrame[playIndex + 1].BRLegs - KeyFrame[playIndex].BRLegs) / i_max_steps;
	KeyFrame[playIndex].BLLegsInc = (KeyFrame[playIndex + 1].BLLegs - KeyFrame[playIndex].BLLegs) / i_max_steps;
...
int main()
{
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].BRLegs = 0;
		KeyFrame[i].BRLegsInc = 0;
		KeyFrame[i].BLLegs = 0;
		KeyFrame[i].BLLegsInc = 0;
		KeyFrame[i].FRLegs = 0;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians(BLLegs), glm::vec3(1.0f, 0.0f, 0.0f)); 
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); 
		B_LeftLeg.Draw(lightingShader);
		//Back Right Leg
		model = modelTemp; 
		model = glm::translate(model, glm::vec3(-0.083f, -0.057f, -0.231f));
		model = glm::rotate(model, glm::radians(BRLegs), glm::vec3(-1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
